...
export class FightManagerService {
    ...
    listenToEvadeResult(): Subscription {
        return this.socketService.on<boolean>(Gateway.GAME, GameEvents.FighterEvade).subscribe((hasEvaded) => {
            if (this.hasEvaded) {
    ...
    listenToNextFightingPlayerTurn(): Subscription {
        return this.socketService.on<string>(Gateway.GAME, GameEvents.StartFightTurn).subscribe((attackerName) => {
