...
export class PlayerMovementService {
    ...
    isPlayerOnIce(node:MovementNode):boolean{
        return this.gameMap.map.mapArray[node.x][node.x].terrain===TileTerrain.ICE;
    }
    ...
    getNeighbors(node: MovementNode, map: Tile[][]): MovementNode[] {
        ...
        if (x < map.length - 1) neighbors.push({ x: x + 1, y: y });
        if (y > 0) neighbors.push({ x: x, y: y - 1 });
        if (y < map[0].length - 1) neighbors.push({ x: x, y: y + 1 });
