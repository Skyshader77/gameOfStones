import { CORRIDOR_OF_WALLS, FOUR_TILED_MOCK_GAMEMAP, MULTIPLE_PLAYERS_PATH, TRAPPED_PLAYER, UNTRAPPED_PLAYER, ZIG_ZAP_PATH } from '@app/constants/test-constants';
...
describe('DijstraService', () => {
    ...
    it('should return a blank array when the player is trapped', () => {
        let gameMap = JSON.parse(JSON.stringify(TRAPPED_PLAYER));
        let currentPlayer = JSON.parse(JSON.stringify(TRAPPED_PLAYER.players[0]));
    ...
    it('should return a blank array when the player wants to move to a tile with a wall', () => {
        let gameMap = JSON.parse(JSON.stringify(CORRIDOR_OF_WALLS));
        let currentPlayer = JSON.parse(JSON.stringify(CORRIDOR_OF_WALLS.players[0]));
        const newPosition: Vec2 = { x: 0, y: 0 };
        expect(service.findShortestPath(newPosition,gameMap,currentPlayer)).toEqual([]);
    });
    ...
    it('should return the sole valid path when the player wants to move through a corridor', () => {
        ...
        expect(service.findShortestPath(newPosition,gameMap,currentPlayer)).toEqual([{ x: 0, y: 1 },{ x: 1, y: 1 },{ x: 2, y: 1 }]);
    });

    it('should return the sole valid path when the player wants to move through a corridor with an open door', () => {
        let gameMap = JSON.parse(JSON.stringify(UNTRAPPED_PLAYER));
        let currentPlayer = JSON.parse(JSON.stringify(UNTRAPPED_PLAYER.players[0]));
        const newPosition: Vec2 = { x: 2, y: 1 };
        expect(service.findShortestPath(newPosition,gameMap,currentPlayer)).toEqual([{ x: 0, y: 1 },{ x: 1, y: 1 },{ x: 2, y: 1 }]);
    });

    it('should return the minimum valid path when the player wants to move through a corridor surrounded by water', () => {
        let gameMap = JSON.parse(JSON.stringify(ZIG_ZAP_PATH));
        let currentPlayer = JSON.parse(JSON.stringify(ZIG_ZAP_PATH.players[0]));
        const newPosition: Vec2 = { x: 2, y: 0 };
        expect(service.findShortestPath(newPosition,gameMap,currentPlayer)).toEqual([{ x: 0, y: 2 },{ x: 1, y: 2 },{ x: 1, y: 1 },{ x: 2, y: 1 }, { x: 2, y: 0 } ]);
    });

    it('should return a blank array when the player wants to move to a tile with a player on it', () => {
        let gameMap = JSON.parse(JSON.stringify(MULTIPLE_PLAYERS_PATH));
        let currentPlayer = JSON.parse(JSON.stringify(MULTIPLE_PLAYERS_PATH.players[0]));
    ...
    it('should return the only possible path when the player wants to move to the furthest away tile', () => {
        ...
        let currentPlayer = JSON.parse(JSON.stringify(MULTIPLE_PLAYERS_PATH.players[0]));
        const newPosition: Vec2 = { x: 1, y: 2 };
        expect(service.findShortestPath(newPosition,gameMap,currentPlayer)).toEqual([{ x: 0, y: 0 },{ x: 1, y: 0 },{ x: 2, y: 0 },{ x: 2, y: 1 }, { x: 2, y: 2 }, { x: 2, y: 1 }]);
    });
